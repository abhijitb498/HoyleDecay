!Decay Simulation for 12C+16O at beam energy 75 MeV
program simulation
implicit none
...
theta3 = theta_min + (theta_max - theta_min)*r  !scattered 12C Polar Angle
phi3 = phi_min + (phi_max - phi_min)*s   !scattered 12C Azimuthal Angle 

!Kinematics
a=1.
b=-(6./7.)*sqrt(E1)*cos(theta3*d2r)
d=(4./7.)*(Ex - 0.24*E1)
...
if ((b**2. - 4*a*d) .gt. 0. .or. (b**2. - 4*a*d) .eq. 0.) then
    E3 = (0.5*(-b + sqrt(b**2. - 4*a*d)))**2.   !scattered 12C KE
    P3 = sqrt(2.*12.*E3)*pol_cart(theta3,phi3)
    !write(*,*)P3
    E4 = E1 - E3 - Ex
    !P4 = sqrt(2.*12.*E4)
    P4 = P1+P2-P3
    !v_C = (P3/12.)!*pol_cart(theta4, phi4) !Velocity of Excited Target 12C nucleus
    v_O = (P4/16.)!Velocity of Excited 16O nucleus
